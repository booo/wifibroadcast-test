...
if __name__ == '__main__':
    ...
    parser.add_argument("-w",
                        "--worktime",
                        help="how long was the datasendtime ?",
                        type=int,
                        default=30)
    parser.add_argument("-r",
                        "--receivers",
                        help="how many receivers are used ?",
                        type=int,
                        default=2)
    ...
    worktime = args.worktime #[sek]
    rxAdapterCount = args.receivers
    debugDataAvali=True
    ...
    for idx,oneTestPosPath in enumerate(testPositions):
        ...
        for oneCsv in csvs:
            ...
            for oneAdapterIdx in range(rxAdapterCount):
                ...
                df_copyT['received_valid_packet_ratio'] = (df_copy['received_packet_cnt'].iloc[-1]-df_copy['wrong_crc_cnt'].iloc[-1]) / df_copy['received_packet_cnt'].iloc[-1] 
                df_copyT['received_valid_block_ratio'] = (df_copy['received_block_cnt'].iloc[-1]-df_copy['damaged_block_cnt'].iloc[-1]) / df_copy['received_block_cnt'].iloc[-1] 
    ...
    for k in range(rxAdapterCount):
        ...
        plot_specific(df_results[k],"received_valid_packet_ratio", "Ant"+str(k))
        if debugDataAvali:
    ...
    plot_specific(df_results[0],"received_valid_block_ratio", "wfb")
